...
\begin{frame}{PathLabel funkciója}
  \begin{itemize}
    \item Meghatározza s csúcs pozícióját t csúcshoz képest az MST-n. (LEFT, RIGHT, ANCESTOR)
    \item s csúcstól kezdve végig megyünk az s $\rightarrow$ t úton, és:
    \begin{itemize}
      \item Ha az adott él még nem volt úttömörítve (v = find(v)), akkor annak az élnek az inputként megadott élt menti el csereélének. Majd úttömörítést hajtunk végre (link(v)).
      \item $k_1$ és $k_2$ változók értékeit módosítja, ameddig végig nem érünk az s $\rightarrow$ t úton vagy nem el nem érjük s és t legalacsonyabb közös ősét (Lowest Common Ancestor, LCA).
    \end{itemize}
  \end{itemize}
